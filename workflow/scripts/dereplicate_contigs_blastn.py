...
def summarize_hits(df_hsps, option_cov="mean", option_pid="mean"):
    ...
    return (
        ...
        pospercentsh,
        evalue,
        np.asarray([df_hsps["qlen"][0], df_hsps["slen"][0]]).argmax(),
        df_hsps["qlen"][0],
...
def summarize_hit_only(split_line, option_cov="mean", option_pid="mean"):
    ...
    return perc_id, cov, evalue, np.asarray([int(split_line[4]), int(split_line[5])]).argmax(), int(split_line[4]), split_line[11]
...
pident = float(snakemake.params.pident)
...
with open(snakemake.output.tsv, "w") as w_file:
    header = "\t".join(
        ["qseqid", "sseqid", "pident", "evalue", "coverage", "longest_len", "qlen"]
    )
    ...
    for sub_blast in iterrator_on_blast_hsp(blast_out=blast_file):
        ...
        if num_HSPs > 0:
            ...
            if (
                ...
                line2write = f"{qseqid}\t{sseqid}\t{pident_blast}\t{evalue_blast}\t{coverage_blast}\t{[qseqid, sseqid][pos_longest_len]}\t{qlen}\n"
                w_file.write(line2write)

###########################################################

# Selecting the longest sequence

blast_out = pd.read_table(snakemake.output.tsv)

blast_out.sort_values("qlen", ascending=False, inplace=True)
...
lines = blast_out.to_dict("records")

for line in lines:
    qseqid = line["qseqid"]
    sseqid = line["sseqid"]

    if (qseqid in remove) or (sseqid in keep):
        continue
    elif qseqid in keep:
        remove.append(sseqid)
    elif qseqid == line["longest_len"]:
        keep.append(qseqid)
        remove.append(sseqid)

with open(snakemake.output.contigs, "wt") as w_file:
    index_seq = SeqIO.index(snakemake.input.contigs, "fasta")

    for name in keep:
        SeqIO.write(index_seq[name], w_file, "fasta")
